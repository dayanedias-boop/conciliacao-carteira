...
App Streamlit com: gráficos, filtros, histórico no GitHub, download Excel
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# GITHUB — salvar e listar arquivos
# ══════════════════════════════════════════════════════════════════════════════
def github_headers():
...
def listar_historico():
    """Lista arquivos da pasta historico/ no repositório."""
    ...
    for a in arquivos:
        if a["name"].endswith(".xlsx"):
            historico.append({
                "nome":         a["name"],
                "url_download": a["download_url"],
                "url_html":     a["html_url"],
                "sha":          a["sha"],
            })
    # Ordenar mais recente primeiro
    historico.sort(key=lambda x: x["nome"], reverse=True)
    return historico

def salvar_no_github(nome_arquivo, excel_bytes, resumo_json):
    """Salva Excel e registro JSON no GitHub."""
    repo = github_repo()
    hdrs = github_headers()

    # 1. Salvar Excel em historico/
    ...
    # Verificar se já existe (para update)
    r_check = requests.get(url_xlsx, headers=hdrs, timeout=10)
    payload_xlsx = {
        "message": f"Conciliação: {nome_arquivo}",
    ...
    # 2. Salvar registro no índice JSON
    ...
    if r_idx.status_code == 200:
        try:
            conteudo = base64.b64decode(r_idx.json()["content"]).decode()
    ...
    payload_idx = {
        "message": f"Índice atualizado: {nome_arquivo}",
...
def carregar_indice():
    """Carrega o índice JSON do histórico."""
    repo = github_repo()
    url = f"https://api.github.com/repos/{repo}/contents/historico/indice.json"
    r = requests.get(url, headers=github_headers(), timeout=10)
    if r.status_code != 200:
        return []
    try:
...
def baixar_excel_github(url_download):
    """Baixa bytes de um Excel salvo no GitHub."""
...
def detectar_aba(file_bytes):
    ...
    wb.close()
    raise ValueError(f"Nenhuma aba com 'id_cobranca' encontrada.")
...
def gerar_excel_bytes(conc,df_ant,df_atu,label_ant,label_atu):
    ...
    gravar_aba_excel(wb,"🔄 Conciliação Geral",conc_mov,
        f"CONCILIAÇÃO GERAL — {label_ant} × {label_atu}","833C00",COLS_G)
    ...
    gravar_aba_excel(wb,"🟡 Alterados",conc_alt,
        f"ALTERADOS — {label_ant} × {label_atu}","833C00",COLS_A)

    conc_nov=conc[conc["tipo_movimentacao"]=="🟢 NOVO"]
    COLS_N=[("id_cobranca","id_cobrança",18,"@","center"),
    ...
    gravar_aba_excel(wb,"🟢 Novos",conc_nov,
        f"NOVOS — {label_atu}","375623",COLS_N)

    conc_sai=conc[conc["tipo_movimentacao"]=="🔴 SAIU DA CARTEIRA"]
    COLS_S=[("id_cobranca","id_cobrança",18,"@","center"),
    ...
    gravar_aba_excel(wb,"🔴 Saíram",conc_sai,
        f"SAÍRAM — {label_ant}","9C0006",COLS_S)
...
with st.sidebar:
    ...
    if not tem_secrets:
        st.warning("Configure GITHUB_TOKEN e GITHUB_REPO nos Secrets do app para ativar o histórico.")
    else:
...
if executar and f_ant and f_atu:
    with st.spinner("Lendo arquivos e cruzando bases..."):
        try:
            ...
            df_ant, _ = ler_base(bytes_ant, f_ant.name)
            df_atu, _ = ler_base(bytes_atu, f_atu.name)
            ...
            if tem_secrets:
                with st.spinner("Salvando no GitHub..."):
                    ...
                    if ok:
                        st.success("✅ Excel salvo no GitHub!")
                    else:
...
if st.session_state.resultado:
    ...
    total_st = contagem["Quantidade"].sum()
    ...
    with col_graf:
        ...
        fig.update_layout(
            ...
            yaxis=dict(tickfont=dict(size=11,family="Inter"),gridcolor="#F0F4FA",zeroline=False),
            bargap=0.35,
        )
    ...
    with col_tab:
        st.markdown("<br>", unsafe_allow_html=True)
        st.dataframe(contagem[["Status","Quantidade","%"]],
    ...
    df_mov = pd.DataFrame({
        "Tipo":       ["🟢 Novos","🔴 Saíram","🟡 Alterados","⚪ Sem Mov."],
        "Quantidade": [n_nov, n_sai, n_alt, n_sem],
        "cor":        ["#1A7F3C","#C92A2A","#E65100","#6B7A99"],
    })
    fig2 = go.Figure(go.Bar(
        x=df_mov["Tipo"], y=df_mov["Quantidade"],
        marker_color=df_mov["cor"].tolist(),
        text=df_mov["Quantidade"].apply(lambda v: f"{v:,}"),
        textposition="outside",
        textfont=dict(size=13,family="Inter, sans-serif"),
    ))
    fig2.update_layout(
        ...
        xaxis=dict(tickfont=dict(size=12,family="Inter"),showgrid=False),
        yaxis=dict(tickfont=dict(size=11,family="Inter"),gridcolor="#F0F4FA",zeroline=False),
        bargap=0.4,
    )
    ...
    if conc_alt.empty:
        ...
        with col_f1:
            campos_sel = st.multiselect(
                "Filtrar por campo alterado", options=todas_opcoes,
                default=[], placeholder="Todos os campos...",
            )
        with col_f2:
            status_opcoes = sorted(conc_alt["status_pagamento_atu"].unique().tolist())
            status_sel = st.multiselect(
                "Filtrar por status atual", options=status_opcoes,
                default=[], placeholder="Todos os status...",
            )
        with col_f3:
            st.markdown("<br>", unsafe_allow_html=True)
        ...
        if campos_sel:
            df_filtrado = df_filtrado[
                df_filtrado["campos_alterados"].apply(lambda x: any(c in x for c in campos_sel))
            ]
        if status_sel:
            df_filtrado = df_filtrado[df_filtrado["status_pagamento_atu"].isin(status_sel)]
        if var_neg:
        ...
        COLS_EX = ["id_cobranca","campos_alterados","status_pagamento_ant","status_pagamento_atu",
                   "valor_pago_ant","valor_pago_atu","var_valor_pago",
                   "valor_excedente_pago_ant","valor_excedente_pago_atu","var_excedente_pago",
                   "contas_a_receber_ant","contas_a_receber_atu","var_contas_a_receber"]
        RENAME = {
            "id_cobranca":"ID Cobrança","campos_alterados":"Campos Alterados",
            "status_pagamento_ant":f"Status ({label_ant})","status_pagamento_atu":f"Status ({label_atu})",
            "valor_pago_ant":"Valor Pago Ant.","valor_pago_atu":"Valor Pago Atu.",
            "var_valor_pago":"Var. Valor Pago",
            "valor_excedente_pago_ant":"Excedente Ant.","valor_excedente_pago_atu":"Excedente Atu.",
            "var_excedente_pago":"Var. Excedente",
            "contas_a_receber_ant":"CAR Ant.","contas_a_receber_atu":"CAR Atu.",
...
st.markdown(
    ...
    "valor_cobranca · valor_pago · valor_excedente_pago · status_pagamento · contas_a_receber"
    "</p>",
    unsafe_allow_html=True,
)
